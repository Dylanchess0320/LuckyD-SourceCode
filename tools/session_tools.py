...
class TodoWriteTool(ToolBase):
    ...
    async def execute(self, todos: list) -> ToolOutput:
        ...
        for tid, t in sorted(_todos.items()):
            ...
            emoji = {"pending": "○", "in_progress": "◉", "completed": "✓", "cancelled": "✗"}.get(st, "?")
...
class ShellHistoryTool(ToolBase):
    name = "ShellHistory"
    description = "Review shell commands (Bash + PowerShell) run during this session. Supports filtering."
    aliases = ["CmdHistory", "History"]
    parameters = {
        "last_n": {"type": "integer", "description": "Only show the last N commands (default: 20, max: 100)"},
        "failed_only": {"type": "boolean", "description": "Only show commands that failed (exit code != 0)"},
        "shell_filter": {"type": "string", "enum": ["all", "bash", "powershell", "cmd"], "description": "Filter by shell type"},
        "search": {"type": "string", "description": "Only show commands containing this substring"},
        "show_output": {"type": "boolean", "description": "Include a preview of each command's output (first 200 chars)"},
    }

    async def execute(
        self, last_n: int = 20, failed_only: bool = False,
        shell_filter: str = "all", search: str = "", show_output: bool = False,
    ) -> ToolOutput:
