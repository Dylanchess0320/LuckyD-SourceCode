...
def _get_jedi():
    try:
        ...
        raise RuntimeError("jedi not installed. Run: pip install jedi")
...
class LspReferencesTool(ToolBase):
    name = "LspReferences"
    description = "Find all references to a symbol. Returns every file and line number that uses it."
...
class LspRenameTool(ToolBase):
    name = "LspRename"
    description = "Safely rename a symbol across all files in the project using LSP reference finding."
    ...
    async def execute(self, file_path: str, line: int, new_name: str, character: int = 0) -> ToolOutput:
        try:
            jedi = _get_jedi()
            path = _resolve_path(file_path)
            source = path.read_text()
            ...
            for fp, file_refs in by_file.items():
                ...
                for r in sorted_refs:
                    ...
                    lines[r.line - 1] = line_str[:r.column] + new_name + line_str[r.column + len(old_name):]
                    changes += 1
                Path(fp).write_text("\n".join(lines) + "\n")
            ...
            return ToolOutput(
                ...
                metadata={"old_name": old_name, "new_name": new_name, "changes": changes, "files": len(by_file)},
            )
        except Exception as e:
...
class LspDocumentSymbolsTool(ToolBase):
    ...
    description = "Get the full symbol outline of a file — all classes, functions, methods, variables."
    ...
    async def execute(self, query: str = "") -> ToolOutput:
        try:
            ...
            for py_file in cwd.rglob("*.py"):
                if any(p.name in {".git", "__pycache__", "node_modules", ".venv"} for p in py_file.parents):
                    continue
                try:
        ...
        try:
            ...
            for py_file in cwd.rglob("*.py"):
                ...
                try:
                    ...
                    pattern = re.compile(rf"class\s+\w+.*:[\s\S]*?def\s+{re.escape(name)}\s*\(", re.MULTILINE)
            ...
            if not found:
                return ToolOutput(text=f"No implementations found for '{name}'.", title="Implementations")
...
class LspIncomingCallsTool(ToolBase):
    ...
    async def execute(self, file_path: str, line: int, character: int = 0) -> ToolOutput:
        try:
            ...
            output = f"Callers of '{name}':\n" + "\n".join(parts) if parts else f"No callers found for '{name}'."
            return ToolOutput(
                text=output,
...
class LspOutgoingCallsTool(ToolBase):
    ...
    async def execute(self, file_path: str, line: int, character: int = 0) -> ToolOutput:
        try:
            ...
            func_source = func.get_line_code()
            # Parse the function body
            callees = set()
            tree = jedi.Script(code=source, path=str(path))
            names = tree.get_names(all_scopes=True)
            for n in names:
                if n.line > func.line and n.line < func.line + 100:  # rough bounds
                    if n.type == "function":
                        callees.add(f"  {n.full_name} — {path.name}:{n.line}")

            output = f"Callees from '{func.name}':\n" + "\n".join(sorted(callees)[:30]) if callees else f"No callees found."
...
# Auto-register
for cls in [
    LspDefinitionTool, LspReferencesTool, LspHoverTool, LspRenameTool,
    LspDocumentSymbolsTool, LspWorkspaceSymbolsTool, LspImplementationTool,
    LspIncomingCallsTool, LspOutgoingCallsTool,
]:
    register_tool(cls())
